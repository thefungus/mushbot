...
function chatHandler(msg) {
    ...
    if ((count + 1) % sett.messages === 0) {
        if (server.size === 1) {
            ...
            msg.client.users.find("id", game[0]).sendMessage(
...
let commands = {
        ...
        code(msg, args, argsStr, cmdName, char) {
            ...
            if (settings.has(msg.guild.id)) {
                ...
                for (let i of channels) {
                    msg.client.channels.find("id", i).sendMessage(
    ...
    acceptmatch: {
        ...
        code(msg, args, argsStr, cmdName, char) {
            ...
            if (server.size === 1) {
                let game = Array.from(server)[0];
                util.getPrivateChannel(msg.client, game[0]).sendMessage(
            ...
            if (server.has(id)) {
                msg.client.users.find("id", id).sendMessage(
