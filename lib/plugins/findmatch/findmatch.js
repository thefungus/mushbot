...
let commands = {
    findmatch: {
        ...
        code(msg, args, argsStr, cmdName, char) {
            if (!settings.has(msg.guild.id) || !settings.get(msg.guild.id).enabled) {
                return;
            }
            let server;
            if (games.has(msg.guild.id)) {
    ...
    togglefindmatch: {
        names: ["togglefindmatch", "togglefm"],
        allowPrivate: false,
        perms: util.MOD,
        code(msg) {
            if (!settings.has(msg.guild.id)) {
            ...
            if (!sett.enabled && games.has(msg.guild.id)) {
                games.get(msg.guild.id).clear();
            }
            saveSettings(msg.guild.id, sett, () => {
                msg.reply(
                    `The findmatch command is now ${sett.enabled ? "enabled" : "disabled"}!`
                );
            });
        }
    }
...
function Settings(prev = {}) {
    ...
    this.enabled = typeof prev.enabled == "boolean" ? prev.enabled : true;
}
