...
function serverHandler(msg, str, sett) {
    ...
    for (let i of str) {
        ...
        let emote = em.get(i);
        msg.channel.sendFile(`${PATH.SERVER(msg.guild.id)}/${emote}`);
        return 1;
    }
}
...
function userHandler(msg, str, sett) {
    ...
    /*let lists = [];
    if (msg.channel.type === "dm") {
        for (let i of msg.client.guilds) {
            if (i.members.exists("id", msg.author.id)) {
                lists.push(i.members);
            }
        }
    } else {
        lists.push(msg.guild.members);
    }
    for (let i of str) {
        for (let users of lists) {
            for (let user of users.values()) {
                if (!emotes.user.has(user.user.id) || sett.disabledUser.indexOf(user.user.id) > -1) {
                    continue;
                }
                let em = emotes.user.get(user.user.id);
                if (!em.has(i)) {
                    continue;
                }
                let emote = em.get(i);
                msg.channel.sendFile(`${PATH.USER(user.id)}/${emote}`);
                return 1;
            }
        }
    }*/
    ...
    for (let i of str) {
        for (let [id, user] of emotes.user) {
            ...
            msg.channel.sendFile(`${PATH.USER(id)}/${emote}`);
            return 1;
        }
    }
}
...
function addEmote(type, id, name, url, callback, secondTry) {
    ...
    let formats = ["gif", "png", "jpeg", "jpg", "webp"];
    let link = url;
    ...
    async.waterfall([
        ...
        (cb) => {
            request.head(link, (err, res) => {
                ...
                if (res.headers) {
                    if (res.headers["content-length"] > 8000000) {
                        return cb("errsize");
                    }
                    if (!res.headers["content-type"]) {
                        return cb();
                    }
                    if (formats.indexOf(mime.extension(res.headers["content-type"])) === -1) {
                        return cb("errtype");
                    }
                }
                return cb();
...
let commands = {
    ...
    listemotes: {
        ...
        code(msg, args, argsStr) {
            ...
            let replies = [];
            replies.push(`${isSelf ? "Your" : `${msg.client.users.find("id", user).username}'s`} emotes (${em.size}):\n`);
            for (let i of em.keys()) {
                replies.push(`\n${i}`);
            }
            replies = util.convertLongMessage(replies);
            for (let i of replies) {
                msg.author.sendMessage(i);
            }
        }
    },
    listserveremotes: {
