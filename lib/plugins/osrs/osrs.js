...
const BF = {
    ...
    BARS_HOUR(type) {
        return Math.floor(BF.MISC.BARS_HR*BF.ORES_TRIP(type)/BF.MISC.ORES_TRIP);
    },
    GP_HOUR(type, ore, bar, coal, stam) {
        return Math.floor((bar-ore-(coal*BF[type].COAL))*BF.BARS_HOUR(type)-BF.MISC.COFFER-7*stam);
    },
    XP_HOUR(type) {
...
function getBFInfo(callback) {
    Async.map(
        ...
        function(err, res) {
            if (err) {
                console.log(err);
                return callback("Error getting the info. Try again later.");
            }
            let ores = {
                Steel: {
                    ore: res[1].buy(),
    ...
    Async.map(
        ...
        function(err, res) {
            ...
            osrs.info(type.BAR)
                ...
                .catch((/*err*/) => {
                    callback("Error getting the info. Try again later.");
                });
        }
    );
}
...
function wikiFormat(name) {
    return `http://oldschoolrunescape.wikia.com/wiki/${name
        .replace(/\s/g, "_")
        .replace(/'/g, "%27")
    }`;
}
...
let commands = {
    ...
    osrsalias: {
        ...
        code(msg, args) {
            ...
            let alias = args[0].toLowerCase().replace(/\W/g, "");
            ...
            itemInfo(item)
                .then(i => {
                    ...
                    saveAlias(alias, id, (err) => {
                        if (err) {
                            console.log(err);
                            msg.reply("There was an error in saving the alias.");
                            return;
                        }
