...
function load(mb, cb) {
    loadSettings(mb, (err) => {
        ...
        mb.on("messageDelete", deleteHandler);
        cb();
    });
}
...
function unload(mb, cb) {
    ...
    mb.removeListener("messageDelete", deleteHandler);
    cb();
}
...
function deleteSetting(id, cb) {
    db.deleteData("reactroles", id, cb);
}
...
function addHandler(reaction, user) {
    ...
    if (sett.emojis[emoji]) {
        reaction.message.guild.fetchMember(user)
            .then((member) => {
                member.addRole(sett.emojis[emoji])
                    .catch(console.log);
            })
            .catch(console.log);
    }
...
function removeHandler(reaction, user) {
    ...
    if (sett.emojis[emoji]) {
        reaction.message.guild.fetchMember(user)
            .then((member) => {
                member.removeRole(sett.emojis[emoji])
                    .catch(console.log);
            })
            .catch(console.log);
    }
...
function deleteHandler(msg) {
    if (!settings.has(msg.id)) {
        return;
    }
    settings.delete(msg.id);
    deleteSetting(msg.id);
}
...
let commands = {
    ...
    mentionrole: {
        ...
        allowPrivate: false,
        perms: util.MOD,
        desc: "Enable temporary mentioning of a role.",
        code(msg, args) {
            if (!args.length) {
                msg.reply("Command usage: !mentionrole (role emoji)");
                return;
            }
            let role = msg.guild.roles.find(val => val.name.toLowerCase() === args.join(" ").toLowerCase());
            if (!role) {
                msg.reply("Role name not found. Make sure it's spelled correctly.");
                return;
            }
            role.setMentionable(true, `${msg.member.displayName} used !mentionrole`);
            msg.reply("Ok, you can mention that role in the next minute.");
            setTimeout(() => {
