...
class TwitchCommand {
    ...
    hasPermission(channel, user) {
        if ((!this.perms || !this.perms.length) &&
            (!settings.channel.has(channel) || this.hasPerm(user, channel, settings.channel.get(channel).commandMode))) {
            return true;
        }
...
let CommandsPlugin = {
    ...
    twitchCommands: {
        ...
        setcommandmode: {
            names: ["setcommandmode", "setcmdmode"],
            perms: ["mod"],
            code(channel, user, args) {
                ...
                let sett;
                if (!settings.channel.has(channel)) {
                    sett = new TwitchSettings();
                } else {
                    sett = settings.channel.get(channel);
                }
                sett.commandMode = args[0];
                twitch.reply(channel, user,
                    "Command mode set to " + args[0]
                );
            }
        }
    }
...
function TwitchSettings(prev = {}) {
    ...
    this.commandMode = prev.commandMode || "user";
}
