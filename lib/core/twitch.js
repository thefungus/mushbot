...
function timeoutUser(channel, user, duration = 600, reason = "mushbot", cb = () => {}) {
    let ch = getChannel(channel);
    let username = user.username || user;
    mush.timeout(ch, username, duration, reason).then(() => {
        cb();
    }).catch((err) => {
        console.log("error timeout user", ch, username, duration, reason);
        cb(err);
    });
}
